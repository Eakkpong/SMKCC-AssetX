...
export default function ExportMockRepairPdfButton() {
  function handleExport() {
    ...
    // Header Text
    doc.setFontSize(22);
    const title = 'รายงานการแจ้งซ่อม วิทยาลัยชุมชนสมุทรสาคร (ข้อมูลจำลอง)';
    doc.text(title, 32, 19);

    // Mock Data
    const data = [
      {
        reported_at: new Date(new Date().setDate(new Date().getDate() - 5)).toISOString(),
        reporter_name: 'สมชาย ใจดี',
        asset_code: '7440-001-0001-1-64',
        issue_description: 'เปิดไม่ติด หน้าจอมืด',
        resolved_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString(),
        status: 'เสร็จสิ้น',
        repair_cost: null,
        external_shop: null
      },
      {
        reported_at: new Date(new Date().setDate(new Date().getDate() - 4)).toISOString(),
        reporter_name: 'สมหญิง รักงาน',
        asset_code: '7110-007-0005-1-67',
        issue_description: 'ปริ้นไม่ออก กระดาษติด',
        resolved_at: new Date(new Date().setDate(new Date().getDate() - 2)).toISOString(),
        status: 'เสร็จสิ้น',
        repair_cost: null,
        external_shop: null
      },
      {
        reported_at: new Date(new Date().setDate(new Date().getDate() - 2)).toISOString(),
        reporter_name: 'อภิชาต ชาติเจริญ',
        asset_code: '7440-001-0007-3-67',
        issue_description: 'ฮาร์ดดิสก์พัง มีเสียงดังแกรกๆ',
        resolved_at: new Date(new Date().setDate(new Date().getDate() - 1)).toISOString(),
        status: 'ส่งซ่อมภายนอก',
        repair_cost: 1500.00,
        external_shop: 'ร้าน JIB สาขาเซ็นทรัล'
      },
      {
        reported_at: new Date(new Date().setDate(new Date().getDate() - 1)).toISOString(),
        reporter_name: 'วิภาดา พารวย',
        asset_code: '7440-001-0002-4-64',
        issue_description: 'แบตเตอรี่เสื่อม เก็บไฟไม่อยู่',
        resolved_at: new Date().toISOString(),
        status: 'ส่งซ่อมภายนอก',
        repair_cost: 2500.00,
        external_shop: 'ศูนย์บริการ Dell'
      }
    ];
    ...
    doc.save(`mock-repair-report.pdf`);
  }

  return (
    <button 
      onClick={handleExport}
      className="flex items-center space-x-2 bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-md font-medium transition"
    >
      <FileText size={20} />
      <span>ดูตัวอย่าง PDF จำลอง</span>
    </button>
  );
}
