...
export default function EquipmentForm({ personnel }: { personnel: any[] }) {
  ...
  if (successCode) {
    ...
    return (
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow-sm border border-gray-200 text-center mt-10">
        ...
        {/* Adjusted Print Area without text, only Logo and smaller size */}
        <div id="print-area" className="mb-6">
          <div className="mx-auto p-2 border border-dashed border-gray-400 inline-flex flex-col items-center justify-center bg-white" style={{ width: '120px' }}>
            <img src="/logo.png" alt="Logo" style={{ width: '40px', marginBottom: '5px' }} />
            <QRCodeSVG value={qrUrl} size={90} />
            <p className="text-[10px] font-bold text-gray-800 mt-1 font-sans">{successCode}</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select name="owner_id" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
